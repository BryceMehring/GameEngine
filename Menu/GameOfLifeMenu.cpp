...
GameOfLifeMenu::GameOfLifeMenu(PluginManager&)
{
...
void GameOfLifeMenu::BuildPluginViewMenu(Game& game, Menu* pMenu)
{
	...
	// Define the Menu
	const PluginManager& pm = game.GetPM();
	const std::vector<DLLType>& PluginKeys = pm.GetPluginKeys();

	for(unsigned int i = 0; i < PluginKeys.size(); ++i)
	{
		RECT R = {100,100 + 50*i,0,0};
		const IPlugin* pPlugin = pm.GetPlugin(PluginKeys[i]);
